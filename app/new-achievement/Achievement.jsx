...
const Achievement = ({ session }) => {
 ...
 const [showModal, setShowModal] = useState(false)
  ...
  return (
    <main className="min-h-dvh md:p-5 p-2 relative">
      ...
      >
        <Form className="max-w-3xl mx-auto my-5 space-y-5">
          ...
          >
            {processing ? <BiLoaderCircle className="animate-spin text-2xl" /> : "Share Achievement"}
          </button>
        </Form>
      </Formik>

    <div className={` ${showModal ? "flex" : "hidden"}  h-dvh w-full bg-black/50 absolute inset-0  items-center justify-center`} >

      <div className="w-80 h-52  bg-white shadow-md relative flex flex-col justify-center gap-5">
    <FaRegCheckCircle className="mx-auto text-green-700 text-5xl"/>
    <p className="text-center text-lg">
      Your achievement has been recorded
    </p>

    <button onClick={()=> setShowModal(false)} className="absolute top-2 right-2 ">
     <IoCloseCircleOutline className="text-4xl text-red-700" />
    </button>
